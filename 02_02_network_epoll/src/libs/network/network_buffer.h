...
#else
#define DEFAULT_SEND_BUFFER_SIZE	(1024 * 128) // 128 kb
...
class NetworkBuffer : public Buffer {
  public:
    explicit NetworkBuffer(unsigned int size);
...
class RecvNetworkBuffer : public NetworkBuffer {
  ...
  private:
    void MemcpyFromBuffer(char* pDst, unsigned int size);
};
...
class SendNetworkBuffer : public NetworkBuffer {
  ...
  private:
    void MemcpyToBuffer(char* pSrc, unsigned int size);
};
